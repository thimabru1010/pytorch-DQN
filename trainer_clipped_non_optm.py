        ...
        for fr in range(pre_fr + 1, self.config.frames + 1):
            ...
            if self.agent.buffer.size() >= self.config.start_training:
                ...
                if self.agent.buffer.size() > self.config.batch_size:
                    ...
                    # For network 1
                    unique, counts = np.unique(index[1], return_counts=True)
                    counts_dict1 = dict(zip(unique, counts))
                    # For network 2
                    unique, counts = np.unique(index[2], return_counts=True)
                    counts_dict2 = dict(zip(unique, counts))
                    if 0 not in counts_dict1.keys():
                        counts_dict1[0] = 0
                    if 1 not in counts_dict1.keys():
                        counts_dict1[1] = 0
                    if 0 not in counts_dict2.keys():
                        counts_dict2[0] = 0
                    if 1 not in counts_dict2.keys():
                        counts_dict2[1] = 0
                    q1_counts1 = counts_dict1[0]
                    q2_counts1 = counts_dict1[1]
                    self.board_logger.scalar_summary('Net1: Frequency Q1', fr, q1_counts1)
                    self.board_logger.scalar_summary('Net1: Frequency Q2', fr, q2_counts1)
                    q1_counts2 = counts_dict2[0]
                    q2_counts2 = counts_dict2[1]
                    self.board_logger.scalar_summary('Net2: Frequency Q1', fr, q1_counts2)
                    self.board_logger.scalar_summary('Net2: Frequency Q2', fr, q2_counts2)


                if fr % self.config.print_interval == 0:
                    print("frames: %5d, reward: %5f, loss1: %4f, loss2: %4f  episode: %4d  \t N1-Q1/Q2: %d/%d  N2-Q1/Q2: %d/%d" % (fr, np.mean(all_rewards[-10:]), loss1, loss2, ep_num, q1_counts1, q2_counts1, q1_counts2, q2_counts2))
